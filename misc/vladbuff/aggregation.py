...
class NetVLAD(nn.Module):
    ...
    def __init__(
        ...
        if self.args.nv_dustbin:
            self.dust_bin = nn.Parameter(torch.tensor(1.0))
        if self.args.useToken_nv:
            self.token_mlp = nn.Sequential(
                nn.Linear(self.dim, 512),
                nn.ReLU(),
                nn.Linear(512, self.args.token_dim),
            )
    ...
    def forward(self, x, wParams=None):
        if self.work_with_tokens:
            x = x.permute(0, 2, 1)
            N, D, _ = x.shape[:]
        else:
            ...
            N, D, H, W = x.shape[:]
        if self.args.antiburst and self.args.ab_upsample:
        ...
        if self.args.nv_dustbin: # use dustin?
            soft_assign = torch.cat([soft_assign, self.dust_bin.expand(soft_assign.shape[0], 1, soft_assign.shape[-1])], 1)
        ...
        if self.args.nv_dustbin: # remove from dustbin
            soft_assign = soft_assign[:, :-1, :]            
        vlad = torch.zeros(
        ...
        #     residual = x_flatten.unsqueeze(0).permute(1, 0, 2, 3) - self.centroids[D : D + 1, :].expand(x_flatten.size(-1), -1, -1).permute(1, 2, 0).unsqueeze(0)
        #     residual = residual * soft_assign[:, D : D + 1, :].unsqueeze(2)
        #     vlad[:, D : D + 1, :] = residual.sum(dim=-1)
        vlad = ((x_flatten[:,None,:,:] - self.centroids[None,:,:,None]) * soft_assign[:, :, None, :]).sum(-1)
        # assert(vlad==vlad2).all()
        
        vlad = F.normalize(vlad, p=2, dim=2)  # intra-normalization
        vlad = vlad.view(N, -1)  # Flatten
        vlad = F.normalize(vlad, p=2, dim=1)  # L2 normalize
        if self.args.useToken_nv:
            vlad = torch.cat([token, vlad], dim=-1)
    ...
    def initialize_netvlad_layer(self, args, cluster_ds, backbone):
        ...
        print(
            f"len(cluster_ds): {len(cluster_ds)} / cluster_ds.dataframe.shape: {cluster_ds.dataframe.shape} total_images: {total_cluster_ds_images}"
        )
        print(
            f"Using len(cluster_ds): {len(cluster_ds)} in SubsetRandomSampler and cluster_ds as dataset"
        )
        ...
        with torch.no_grad():
            ...
            for iteration, (inputs) in enumerate(tqdm(random_dl, ncols=100)):
                ...
                # print(f"Iteration {iteration}: Number of descriptors received: {inputs.shape[0]}")
                ...
                # print(f"Iteration {iteration}: Number of descriptors after processing: {image_descriptors.shape[0]}")
                # print(f"image_descriptors shape: {image_descriptors.shape}")
                ...
                # print(f"Iteration {iteration}: Descriptors shape after populating: {descriptors.shape}")
        print(f"descriptors shape: {descriptors.shape}")
        print(
            f"Number of negative values in descriptors: {np.count_nonzero(descriptors < 0)}"
        )
        print(f"descriptors contains NaN: {np.isnan(descriptors).any()}")
        print(f"descriptors contains Inf: {np.isinf(descriptors).any()}")

        if args.nv_pca is not None:
            print(f"====NV PCA {args.nv_pca}...===")
            if args.nv_pca_alt:
                print(f"==== PCA ALT {args.nv_pca_alt}...===")
                w, b = (
                ...
                print(f"==== PCA MLP {args.nv_pca_alt_mlp}...===")
                descriptors = self.mlp(torch.from_numpy(descriptors)).detach().cpu().numpy()
                pcaData = None
            else:
