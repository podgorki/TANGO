...
class MatchLightGlue:
    ...
    def map_node2kp(self, kp, nodeInds):
        ...
        if masks.shape[1] != self.resize_h or masks.shape[2] != self.resize_w:
            masks = cv2.resize(masks.transpose(1, 2, 0).astype(float), (self.resize_w, self.resize_h),
    ...
    def matchPair_imgWithMask(self, imSrc, imTgt, nodesSrc, nodesTgt, visualize=False):
        ...
        if visualize:
            ...
            for i in range(len(mkp1)):
                cv2.line(im_lfm, (int(mkp1[i][0]), int(mkp1[i][1])),
                         (int(mkp2[i][0] + imSrc.shape[1]), int(mkp2[i][1])), (255, 0, 0), 2, lineType=cv2.LINE_AA)
            ...
            for i in range(matchesBool.sum()):
                cv2.line(im_lfm_nodes, (int(coords_i[i][0]), int(coords_i[i][1])),
                         (int(coords_j[i][0] + self.resize_w), int(coords_j[i][1])), (255, 0, 0), 2,
    ...
    def matchPair_imgWithMask_multi(self, qryImg, refImgList, qryNodes, refNodesList, visualize=False):
        ...
        for i in range(len(refImgList)):
            ...
            matchesBool, matches_ij, _, _, vizImgs = self.matchPair_imgWithMask(qryImg, refImg, qryNodes, refNodes,
                                                                                visualize)
...
if __name__ == "__main__":
    ...
    matchesBool, matches_ij, singleBestMatch, _, vizImgs = matcher.matchPair_imgWithMask(imgPath1, imgPath2, masks1,
                                                                                         masks2, visualize=True)
